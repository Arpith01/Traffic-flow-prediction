...
inputCSVPath = "./data/raw/yellow_tripdata_2009-01_point.csv"
outputDirpath = "./data/processed/"
...
def roundDateTime(dt) :
    ...
    date = dt[0].replace('-', "")
    time = dt[1].split(':')
    print("minuteWindow: ",minuteWindow)
    if (isHourWindow):
        time[1] = '00'
        hour = int(hourWindow * (int(time[0]) // hourWindow))
        if hour < 10:
            time[0] = '0' + str(hour)
        else:
            time[0] = str(hour)
    elif (isMinuteWindow):
        minute = int(minuteWindow * (int(time[1]) // minuteWindow))
        if minute < 10:
            time[1] = '0' + str(minute)
        else:
            time[1] = str(minute)
...
def writeOutputToFile(startResults, endResults):
    ...
    for outputFileName in startResults:
        ...
        # np.save(startOutputDir+ "/" + outputFileName,startResults[outputFileName])
        # np.save(endOutputDir+ "/" + outputFileName ,endResults[outputFileName])
